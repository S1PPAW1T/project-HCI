  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/user/login`, {
        ...
        body: JSON.stringify({ name, age: parseInt(age, 10), consent }),
      });
      ...
      localStorage.setItem("userInfo", JSON.stringify({ name: data.data.name, age: data.data.age }));
      ...
      <main className="flex flex-col items-center justify-center flex-1 w-full px-6 pt-2 pb-8">
        <div className="space-y-6 w-full">
          <div className='bg-[#F0F0F0] px-4 py-4 rounded-xl'>
            <h2 className="text-xl font-bold text-black mb-4 text-center">Informed Consent</h2>
            <ul className="space-y-5 text-sm text-black">
              <li className="grid grid-cols-[90px_1fr] gap-4">
                <span className="font-bold">Purpose:</span>
                <span>I understand that my voice will be recorded and used as primary data for this research study.</span>
              </li>
              <li className="grid grid-cols-[90px_1fr] gap-4">
                <span className="font-bold">Privacy:</span>
                <span>My data will remain anonymous and be stored securely for academic purposes only.</span>
              </li>
              <li className="grid grid-cols-[90px_1fr] gap-4">
                <span className="font-bold">Voluntary:</span>
                <span>I acknowledge that I can withdraw from the study at any time without any penalty.</span>
              </li>
            </ul>
            <div className="mt-6 flex items-start gap-3">
              ...
              <label htmlFor="consent" className="text-sm text-[#7018B3] cursor-pointer">
                I have read and understood the terms and conditions<span className="text-red-500">*</span>
              </label>
            </div>
          </div>

          <div>
            <h2 className="text-xl font-bold text-black mb-4">Participant Information</h2>
            <p className="text-sm text-black mb-6">Please provide your information to proceed:</p>
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  required
                />
                <p className="text-xs text-[#B47FDC] mt-1">* To protect your privacy, you may use a nickname or initials</p>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-4 py-2 text-sm rounded-lg border bg-purple-100 text-black placeholder-gray-500 focus:outline-none focus:ring-2 transition-colors ${age && !isValidAge ? "border-red-500 focus:ring-red-500" : "border-purple-200 focus:ring-purple-500"}`}
                  required
                />
                {age && !isValidAge && (
                  <p className="text-xs text-red-500 mt-1">Age must be between 18 and 25 years old.</p>
                )}
              </div>
            </div>
          </div>
